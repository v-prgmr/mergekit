...
# Create a Phi2 MoE from a set of equally-sized Phi2 models.
...
def is_bad_config(config: Phi2MOEConfig, allow_all_same: bool = False) -> bool:
...
def build(
    ...
    for tensor_name in arch_info.pre_weights() + arch_info.post_weights():
        ...
        for layer_idx in range(base_cfg.num_hidden_layers):
            ...
            if ".mlp." in tensor_name:
                for moe_index, expert in enumerate(config.experts):
                    if tensor_name in seen_tensor_names:
                        expert_name = tensor_name.replace(
                            ".mlp.fc", f".moe.mlp.{moe_index}.fc"
                        )
                    else:
                        expert_name = tensor_name.replace(
                            ".mlp.fc", f".moe.mlp.{moe_index}.fc"
                        )
                        seen_tensor_names.add(tensor_name)
...
@click.command("mergekit-moe-phi2")
...
def main(
    ...
    config = Phi2MOEConfig.model_validate(yaml.safe_load(config_source))
    build(
        config,
        out_path=out_path,
